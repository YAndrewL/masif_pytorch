# -*- coding: utf-8 -*-
'''
@File   :  computePolar.py
@Time   :  2023/12/11 12:01
@Author :  Yufan Liu
@Desc   :  Compute polar coordinates
'''
...
def generate_polar_coords(mesh, do_fast=True, radius=12, max_vertices=200):
    ...
    print('number of v', len(vertices))
    print('number of f', len(faces))
    ...
    D = dict_to_sparse(d2)
    print("Dijkstra: ", time.time() - time_tic)
...
def compute_theta_all_fast(D, vertices, faces, normals, idx, radius):
    ...
    """
    mymds = MDS(n_components=2, n_init=1, 
                eps=0.1, max_iter=50, 
