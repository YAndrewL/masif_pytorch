# -*- coding: utf-8 -*-
'''
@File   :  chemistry_helper.py
@Time   :  2023/12/09 11:32
@Author :  Yufan Liu
@Desc   :  Chemistry definitions and help functions
'''

# lyf codebase from xcc
# from rdkit import Chem
# from gemmi import cif
# file_path="/ibex/user/xuc0d/data/CPEPAF/ligand/"
# files=os.listdir("luna18")
# def load_resi_cif_file(path):
#     parser = cif.read_file(path)
#     block = parser.sole_block()    
#     ret = {}

#     ret["Atoms"] = {}
    
#     for aid, aaid, atype, ax, ay, az, aord in zip(block.find_loop("_chem_comp_atom.atom_id"), block.find_loop("_chem_comp_atom.alt_atom_id"), block.find_loop("_chem_comp_atom.type_symbol"), block.find_loop("_chem_comp_atom.pdbx_model_Cartn_x_ideal"), block.find_loop("_chem_comp_atom.pdbx_model_Cartn_y_ideal"), block.find_loop("_chem_comp_atom.pdbx_model_Cartn_z_ideal"), block.find_loop("_chem_comp_atom.pdbx_ordinal")):
#         aaid = aaid.replace("\"", "")
#         aid = aid.replace("\"", "")
#         assert aid not in ret["Atoms"]
#         ret["Atoms"][aid] = {
#             "alt_name": aaid,
#             "type": atype,
#             "cord": [float(ax) if ax != "?" else -1, float(ay) if ay != "?" else -1, float(az) if az != "?" else -1],
#             "order_start1": int(aord)
#         }
...
# def get_h_name(res):
#     ret=[]
#     mol=Chem.MolFromMolFile(os.path.join(file_path, res+"_ideal.sdf"), sanitize=False)
#     cifheader=load_resi_cif_file(os.path.join(file_path, res+".cif"))
#     rev_dict={cifheader["Atoms"][_]["order_start1"]-1:_ for  _ in cifheader["Atoms"]}
#     for atom in mol.GetAtoms():
#         if atom.GetSymbol()=="H":
#             neighbor=atom.GetNeighbors()
#             assert len(neighbor)==1
#             if neighbor[0].GetSymbol()!="C":
#                 ret.append(rev_dict[atom.GetIdx()])
#     return ret
# res={}
# for v in files:
#     res[v]=get_h_name(v)
